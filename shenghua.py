...
def sampleresults(tenant, tenancyName, date, base, status):
    ...
    if status == 1:
        ...
        if '卫光' in tenancyName:
            altsql = "INSERT INTO dbo.OriginalResult (EquipmentName, DataSource, DataFlag, Status, TenantId, CreationTime, CreatorUserId, LastModificationTime, LastModifierUserId) VALUES (N'MR-BS820M', N'MSH|^~\&|||||" +time.strftime("%Y%m%d%H%M%S", time.localtime())+ "||ORU^R01|68|P|2.3.1||||0||ASCII|||PID|" +date["PlateNo"]+ "|||||||O|||||||||||||||||||||||OBR|" +date["PlateNo"]+ "|" +date["Code"]+ "|65|^|N|" +time.strftime("%Y%m%d%H%M%S", time.localtime())+ "|" +time.strftime("%Y%m%d%H%M%S", time.localtime())+ "|" +time.strftime("%Y%m%d%H%M%S", time.localtime())+ "||9^1^1^N0009||||" +time.strftime("%Y%m%d%H%M%S", time.localtime())+ "|???|||||||||||||||||||||||||||||||||OBX|1|NM|ALT|????????????|" +date["ALT"]+ "|U/L|-|N|||F||29.611302|"+ time.strftime("%Y%m%d%H%M%S", time.localtime()) +"|||0|||M1|2008:AC9E#2008:8000|OBX|2|NM|TP|?????(????巨)|" +date["TP"]+ "|g/L|-|N|||F||62.869764|"+ time.strftime("%Y%m%d%H%M%S", time.localtime()) +"|||0|||M1|2001:9A0E#2001:C04E|', N'', 0, " + tenant + ", getdate(), null, getdate(), null);"
        elif '丹霞' in tenancyName:
            altsql = "INSERT INTO dbo.OriginalResult (EquipmentName, DataSource, DataFlag, Status, TenantId, CreationTime, CreatorUserId, LastModificationTime, LastModifierUserId) VALUES (N'MR-BS360s', N'MSH|^~\&|||||" +time.strftime("%Y%m%d%H%M%S", time.localtime())+ "||ORU^R01|68|P|2.3.1||||0||ASCII|||PID|" +date["PlateNo"]+ "|||||||O|||||||||||||||||||||||OBR|" +date["PlateNo"]+ "|" +date["Code"]+ "|65|^|N|" +time.strftime("%Y%m%d%H%M%S", time.localtime())+ "|" +time.strftime("%Y%m%d%H%M%S", time.localtime())+ "|" +time.strftime("%Y%m%d%H%M%S", time.localtime())+ "||9^1^1^N0009||||" +time.strftime("%Y%m%d%H%M%S", time.localtime())+ "|???|||||||||||||||||||||||||||||||||OBX|1|NM|ALT|????????????|" +date["ALT"]+ "|U/L|-|N|||F||29.611302|"+ time.strftime("%Y%m%d%H%M%S", time.localtime()) +"|||0|||M1|2008:AC9E#2008:8000|OBX|2|NM|TP|?????(????巨)|" +date["TP"]+ "|g/L|-|N|||F||62.869764|"+ time.strftime("%Y%m%d%H%M%S", time.localtime()) +"|||0|||M1|2001:9A0E#2001:C04E|', N'', 0, " + tenant + ", getdate(), null, getdate(), null);"
